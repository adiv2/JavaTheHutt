	...
	void showBoard3()
{
	...
	public int rounds=0,h=0;
	int[][] board3 = new int[9][9];
	int fire(ships s1, int i, int j) throws java.lang.Exception
	{
 ...
 {
	 private int i,j,luck;
	 int setNavy() throws java.lang.Exception
	 {
		 int size =3;
		for(int a=0;a<3;a++)
		{
        ...
        if((aim.length())>4){System.out.println("Error!"); setNavy();}
        else
        {
        i=  aim.charAt(0)- 65;
        j= aim.charAt(1)-48;
        luck = aim.charAt(3) - 48;
        setShip(size,i,j,luck);
		size++;
		showBoard();
		}
...
{
	int gameplay(ships s1, navy n1,int i,int j) throws java.lang.Exception
	{
		while(winner!=100)
		{
