...
public class OCV_1
{
    ...
    private String readPath ="/home/aditya/Desktop/C2";
    ...
    protected ArrayList<String> nonMatchList = new ArrayList<>();
    protected ArrayList<String> matchList = new ArrayList<>();
        ...
        for(File file:files)
        {
            ...
            if(dense<512)
            {
               if(match())
               {
                   ...
                   writeMat(source,"/home/aditya/Desktop/C2/"+fileName);
               }
               else {System.out.println("Added to non match \n");nonMatchList.add(fileName);dense=0;templateList.clear();}
            }
            if(dense>512)
            {
    ...
    protected void writeMat(Mat destination,String path)
    {
        ...
        try
        {
            //ImageIO.write(bufferedImage1, "png", op);
            ImagePlus w = new ImagePlus("w",bufferedImage1);
            IJ.saveAs(w,"png",path);
        }
    ...
    {
        Mat r2 = new Mat();
        String refFile = "/home/aditya/Desktop/t/A/ref_"+fileName;
    ...
    {
        int i=0;
        File[] files = new File("/home/aditya/Desktop/t/RefsCloud/").listFiles();
        ...
        matches2=matches;
        Arrays.sort(mval);
        System.out.print(fileName +" dense:"+dense+" match:"+matches2+" Notmatch "+notMatch+" ");
        if (mval.length>10)
        {
        ...
        if(matches>=(dense*0.1) && matches>=10)
        {
    ...
    void verify()
    {
        ...
        try
        {
            allList  = (ArrayList<String>) FileUtils.readLines(file, Charset.defaultCharset());
        }
        catch (Exception e){}
        ...
        {
            String fileName = file2.getName().replace(".jpeg","");
        ...
        System.out.println("Non matches which have clouds");
        for (String nonmatch:nonMatchList)
        {
            for(String check:checkList)
            {
                if(check.contains(nonmatch) && check.contains("clouds"))
                {
                    System.out.println("Log says "+nonmatch+" contains clouds");
                }
            }
        }
        System.out.println("Matches which have clouds but not checked");
        for (String match:matchList)
        {
            for(String check:checkList)
            {
                if(check.contains(match) && check.contains("clouds"))
                {
                    System.out.println("Log says "+match+" contains clouds");
                }
            }
        }
